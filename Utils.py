...
from GameObject import GameObjectCorner
...
def generateResizedImage(basePath, imageName, newWidth = -1, newHeight = -1):
    ...
    if imageName == Constants.PIPE_NORTH_NAME:
        ...
        croppedImgName = basePath + Constants.RESIZE_IMAGE_DIRECTORY_PREFIX + getUuid() + getFileExtension(imageName);
    ...
    resizedImageName = resizeImage(basePath,
                                   imageName,
                                   Constants.RESIZE_IMAGE_DIRECTORY_PREFIX + getUuid(),
                                   newWidth,
                                   newHeight)
    return resizedImageName

def image_resize(basePath, imageName, resizedImageName, width, height, inter = cv2.INTER_AREA):
    ...
    resizedImagePath = basePath + resizedImageName + getFileExtension(imageName)
...
def resizeImage(basePath, imageName, resizedImageName, dimX, dimY):
    return image_resize(basePath, imageName, resizedImageName, dimX, dimY)
...
def getGameObjectCornerCoordinates(gameObject, corner):
    if corner == GameObjectCorner.TOP_LEFT:
        return gameObject.xCoordinate, gameObject.yCoordinate
    if corner == GameObjectCorner.TOP_RIGHT:
        return gameObject.xCoordinate + gameObject.objectWidth, gameObject.yCoordinate
    if corner == GameObjectCorner.BOTTOM_LEFT:
        return gameObject.xCoordinate, gameObject.yCoordinate + gameObject.objectHeight
    if corner == GameObjectCorner.BOTTOM_RIGHT:
        return gameObject.xCoordinate + gameObject.objectWidth, gameObject.yCoordinate + gameObject.objectHeight
